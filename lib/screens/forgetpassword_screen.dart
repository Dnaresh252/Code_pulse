...
//     _emailFocusNode.addListener(() {
//       if (!_emailFocusNode.hasFocus) {
//         // Clear error when user finishes typing
//         if (_errorMessage != null) {
//           setState(() {
//             _errorMessage = null;
//           });
...
//       final currentTime = DateTime.now().millisecondsSinceEpoch;
//       final timeDifference = currentTime - lastAttemptTime;
//       final cooldownTime = _cooldownMinutes * 60 * 1000; // Convert to milliseconds
//
//       // If cooldown period has passed, reset attempt count
//       if (timeDifference > cooldownTime) {
//         await prefs.remove('password_reset_attempt_count');
//         await prefs.remove('last_password_reset_attempt');
//         await prefs.remove('last_password_reset_email');
...
//       } else {
//         _attemptCount = attemptCount;
//         // Pre-fill email if it's the same user
//         if (lastAttemptEmail.isNotEmpty) {
//           _emailController.text = lastAttemptEmail;
//         }
//       }
...
//         setState(() {
//           _errorMessage = "No account found with this email address. Please check your email or create a new account.";
//         });
//         _attemptCount++;
//         await _saveAttemptData(email);
//         return;
...
//             setState(() {
//               _errorMessage = "This account has been deactivated. Please contact support for assistance.";
//             });
//             return;
//           }
//
...
//             setState(() {
//               _errorMessage = "Please verify your email first before resetting password. Check your inbox for verification email.";
//             });
//             return;
//           }
//
...
//         // Log Firestore error but don't block the password reset
//         debugPrint('Firestore check failed: $firestoreError');
//         // Continue with password reset even if Firestore check fails
//       }
//
//       // Step 3: Send password reset email with custom settings
//       await _auth.sendPasswordResetEmail(
//         email: email,
//         actionCodeSettings: ActionCodeSettings(
//           url: 'https://yourapp.com/login', // ✅ Replace with your app's login URL
//           handleCodeInApp: false,
//           androidPackageName: 'com.yourcompany.yourapp', // ✅ Replace with your package name
//           iOSBundleId: 'com.yourcompany.yourapp', // ✅ Replace with your bundle ID
...
//         _emailSent = true;
//         _successMessage = "Password reset link sent successfully!";
//         _attemptCount++; // Increment for successful attempts too
//       });
//
//       await _saveAttemptData(email);
//       _successController.forward();
...
//     } catch (e) {
//       setState(() {
//         _errorMessage = 'Network error occurred. Please check your connection and try again.';
//       });
...
//         return 'This account has been disabled. Please contact support.';
//       case 'too-many-requests':
//         return 'Too many reset attempts. Please wait $_cooldownMinutes minutes before trying again.';
//       case 'network-request-failed':
//         return 'Network error. Please check your connection and try again.';
//       case 'weak-password':
//         return 'Please choose a stronger password when resetting.';
//       case 'requires-recent-login':
//         return 'Please log in again before requesting a password reset.';
//       case 'quota-exceeded':
//         return 'Password reset quota exceeded. Please try again later.';
//       case 'invalid-action-code':
//         return 'The reset link is invalid or has expired.';
//       case 'expired-action-code':
//         return 'The reset link has expired. Please request a new one.';
//       default:
//         return 'Failed to send reset link. Please try again or contact support.';
//     }
//   }
//   // ✅ Resend timer functionality with proper cleanup
//   void _startResendTimer() {
//     setState(() {
//       _canResend = false;
//       _resendTimer = _resendCooldownSeconds;
//     });
//
//     _resendTimerInstance?.cancel(); // Cancel any existing timer
//     _resendTimerInstance = Timer.periodic(const Duration(seconds: 1), (timer) {
//       if (mounted) {
//         setState(() {
//           _resendTimer--;
...
//     if (!_canResend) return;
//
//     // Check if we're still within rate limits
//     if (_isRateLimited()) return;
//
//     await _sendPasswordResetEmail();
//   }
...
//   }
//
//   // ✅ Handle back navigation with confirmation if email was sent
//   Future<bool> _onWillPop() async {
//     if (_emailSent) {
//       final shouldPop = await showDialog<bool>(
//         context: context,
